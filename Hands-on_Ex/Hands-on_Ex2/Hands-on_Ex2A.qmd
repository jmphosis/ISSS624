...
In this page, I show how I had completed the Hands-on Exercise 2A, on computing spatial weights using R.
...
These two sub-folders are within the data folder of my Hands-on_Ex2A folder.
...
***Student Note***: Without stating the "by" argument for left_join(), the two tables are joined by the 'County' columns.
...
The mapping function, **map_dbl()** (under the purrr package) is utilised to apply a function, **st_centroid()**, on each element of the geometry column, `us.bound`, returning a vector of a same length.
...
Firstly, to determine the upper limit for the distance band by using the following steps and functions under the **spdep** package:
...
[**\~\~\~ End of Hands-on Exercise 12A \~\~\~**]{.smallcaps}
