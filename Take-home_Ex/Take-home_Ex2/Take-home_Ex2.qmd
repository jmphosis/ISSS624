...
2.  For commuter flows between hexagons that are next to each other, the Government may wish to study if the provision of more sheltered walkways or cycling paths may encourage commuters to walk or cycle to the MRT stations instead of relying on buses for the short commutes.
...
-   Based on the outcome from the `cor()` function in the **R stats** package, the R-squared value is 0.09206193. This means that about 9.25% of the variation in the number of trips can be accounted for by the unconstrained SIM.
...
-   Based on the outcome from the `cor()` function in the **R stats** package, the R2 is 0.1397128. This means that about 14.0% of the variation in the number of trips can be accounted for by the origin constrained SIM.
...
-   Based on the outcome from the `cor()` function in the **R stats** package, the R2 is 0.1934442. This means that about 20.3% of the variation in the number of trips can be accounted for by the destination constrained SIM.
